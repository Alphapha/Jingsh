...
<%
	...
	if (rs.bof and rs.eof) then'查找表判断是否已入职
		response.write "<script>alert('你不满足办卡条件,请联系相关负责人处理后再来办理');location.href='../aadmin/hr_lawyer.asp'</script>"
		response.end
	else
		response.write "<script>alert('审核通过,进入下一步');location.href='../menjin/safe_pay.asp'</script>"
	end if
%>
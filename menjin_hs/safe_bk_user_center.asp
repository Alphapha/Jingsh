...
<div class="container" style="background: url(../img/hs/logo_hs1.png) no-repeat ;background-size: 100% auto;height: 650px;width: 100%;max-width: 600px; " >
	<div class="row clearfix" style="margin: 10px 2%; opacity: 1;>
		<div class="col-md-12 column" style="position: relative;padding: 5px 0 0 5px;opacity: 1;">
			<%
				...
				sql = "select * from hs_safe where 姓名='"&menjin_name&"' and 手机 ='"&menjin_phone&"' and 服务类型='补卡' order by 门禁卡办理时间 desc"
				rs.open sql,conn,1,3
				if login<>"ok" then response.redirect "../hs.html"
			%>
			<div >
				<div>
					<h3>尊敬的&nbsp;</h3><h2><%=rs("姓名")%></h2><h3>&nbsp;先生/女士：</h3>
				</div><br>
				<h4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您于&nbsp;<b><%=rs("门禁卡办理时间")%></b>&nbsp;在沪师上海总部办理了门禁一卡通补卡业务，补卡卡号为&nbsp;<b><%=rs("编号_门禁卡")%></b>&nbsp;，补卡支付凭证号为&nbsp;<b><%=rs("费用_门禁卡")%></b>&nbsp;，补卡绑定的手机号为&nbsp;<b><%=rs("手机")%></b>&nbsp;，此界面截图作为补卡凭证，领取门禁卡时请出示此截图。</h4><br><br>
					<h5>随同附上</h5>
					<h5>我衷心希望</h5>
					<h5>您永远用不到的</h5>
					<h5>温馨小贴士：</h5>
